...
nal_short_info h264Decoder::parseNAL(raw_data nal)
{
	...
	if (hdr.nal_unit_type == H264Parser::NAL_T_PREFIX_NAL_UNIT_RBSP) {
		...
		// This check is there because we do not want to increase the frame counter
		// for the NAL units after the meta-data NAL units.
		if (last_prefix_nal_.nal_nr != -1) {

			// Multiple NALs of the same layer in a row are counted as
			// one frame.
			if (nal_info.dependency_id != last_prefix_nal_.dependency_id ||
				nal_info.temporal_id != last_prefix_nal_.temporal_id ||
				nal_info.quality_id != last_prefix_nal_.quality_id) {
